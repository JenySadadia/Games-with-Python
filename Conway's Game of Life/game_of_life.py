...
class Board(object):
    ...
    def __init__(self, win, width, height):
        ...
        for i in range(13):
            for j in range(13):
                self.block_list[(i,j)]=Block(Point(i,j),color_rgb(random.randint(0,255),random.randint(0,255),random.randint(0,255)))
    ...
    def seed(self, block_coords):
        ...
        for block in block_coords:
            a=self.block_list[block]
            a.set_live(self.canvas)
    ...
    def get_block_neighbors(self, block):
        ...
        '''
        l=[(block.x,block.y-1),(block.x-1,block.y-1),(block.x+1,block.y-1),(block.x-1,block.y),(block.x+1,block.y),(block.x-1,block.y+1),(block.x,block.y+1),(block.x+1,block.y+1)]
        m=[]
        for i in l:
            if 13>i[0]>-1 and 13>i[1]>-1:
                n=self.block_list[i]
                m.append(n)
        return m
    ...
    def simulate(self):
        ...
        def set_newStatus():
            ...
            for i in self.block_list.values():
                count=0
                l=board.get_block_neighbors(i)

                for j in l:
                    check_live=j.is_live()
                    if check_live==True:
                        count+=1
                #print count,"No of live neighbour cell"
...
if __name__ == '__main__':    
    ...
    block_coords=board.block_list.keys()
